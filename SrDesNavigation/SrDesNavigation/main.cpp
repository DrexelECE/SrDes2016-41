...
int main(int argc, char** argv)
{
	try {
		...
		//host = "10.0.0.10";// :8000";
		host = "10.0.0.10";
		con_http con(host, 8000);
//		con.updatePWM();
		std::vector<char> buffer;
		con.getImage(buffer);
		cv::Mat image(buffer);
		image = cv::imdecode(image, CV_LOAD_IMAGE_COLOR);
...
//	}
	...
	//while(nonterminated){
		//con_http con(argv[1], argv[2]/*);
		//con.getImage(cameraNumber, somesstringforfilepath/);

	image = cv::imread(somesstringforfilepath, CV_LOAD_IMAGE_COLOR);   // Read the file
	...
	Nav::patternRec(bw, RGBchannels, pattern, centroid);
		...
		//Nav::pwmTransform(centroid,cameraNumberOut, pwmArray);
		//con.updatePWM(pwmArray);
	//}
